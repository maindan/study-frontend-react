            ...
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              ...
              <button
                onClick={async () => {
                  ...
                  const canProceed = onNext ? await onNext(currentStep) : true;
                  if (canProceed) {
                ...
                className={`next-button flex items-center justify-center gap-2 ${isNextDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                {...nextButtonProps}
              >
                {isNextLoading ? (
